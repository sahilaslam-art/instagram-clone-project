...
export default function CustomerSupport() {
  ...
  const [ticketForm, setTicketForm] = useState({ ticketSubject: '', ticketCategory: 'Investment Issue', ticketDescription: '' });
  ...
  const handleSubmitTicket = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await api.post('/customer/support/tickets', ticketForm);
      setTicketForm({ ticketSubject: '', ticketCategory: 'Investment Issue', ticketDescription: '' });
        ...
        {tab === 'FAQ' && (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {[
                { q: 'How does the investment process work?', a: 'You browse live projects, review their details, and invest using your wallet balance. Once the project completes its funding stage, it becomes live.' },
                { q: 'How do I withdraw my returns?', a: 'Returns are credited to your wallet upon project completion. You can request a withdrawal from the My Wallet section.' },
                { q: 'Are my investments guaranteed?', a: 'No, all investments carry some level of risk. We display a risk level for each project to help you make informed decisions.' }
              ].map((faq, i) => (
        ...
        {tab === 'PRIVACY' && (
          <div className="space-y-6 prose prose-blue max-w-none">
            ...
            <div>
              ...
              <p className="text-gray-600">We value your privacy and are committed to protecting your personal data. This policy outlines how we collect and manage your information.</p>
            </div>
            <div>
              <h3 className="text-lg font-semibold text-gray-800">Information We Collect</h3>
              <p className="text-gray-600">We collect profile details, KYC documents, and investment activity to ensure platform security and regulatory compliance.</p>
            </div>
            <div>
              <h3 className="text-lg font-semibold text-gray-800">How Information is Used</h3>
              <p className="text-gray-600">Your data is used strictly for account verification, processing investments, and communicating important updates regarding your portfolio.</p>
            </div>
          </div>
        )}
                    ...
                    <div key={ticket._id} className="p-4 border border-gray-200 rounded-lg bg-gray-50">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                          ...
                          ticket.ticketStatus === 'In Progress' ? 'bg-emerald-100 text-emerald-700' :
