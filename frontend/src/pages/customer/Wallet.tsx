...
export default function CustomerWallet() {
  ...
  const handleAdd = async () => {
    ...
    if (val > 0) {
      try {
        setLoading(true);
        const res = await api.post('/customer/wallet/add-funds', { amount: val });
  ...
  const handleWithdraw = async () => {
    ...
    if (val > 0) {
      if (val > (currentUser.walletBalance || 0)) {
        ...
        try {
          setLoading(true);
          const res = await api.post('/customer/wallet/withdraw', { amount: val });
          updateUser({ walletBalance: res.data.data.wallet.availableBalance });
          setAmount('');
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500">Manage your available balance and transactions.</p>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 bg-white/10 p-4 rounded-xl backdrop-blur-sm">
          ...
          <div className="flex gap-2">
            <button 
              onClick={handleAdd}
              disabled={loading}
              className="flex-1 sm:flex-none px-6 py-3 bg-white text-emerald-700 hover:bg-gray-50 rounded-lg font-medium transition-colors flex items-center justify-center gap-2 disabled:opacity-70"
            >
